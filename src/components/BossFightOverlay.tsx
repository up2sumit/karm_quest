...
export function BossFightOverlay({ open, onClose, rewardCoins }: BossFightOverlayProps) {
  ...
  useEffect(() => {
    ...
    // Explode → reward + confetti
    const tReward = window.setTimeout(() => {
      setPhase('reward');
      setShowConfetti(true);
    }, 450 + 520 * 6 + 1100);

    // Focus close button once card is visible
  ...
  const accent = isHinglish ? 'from-rose-500 via-violet-500 to-indigo-500' : 'from-indigo-500 via-violet-500 to-purple-600';
  const title = isHinglish ? '🔥 BOSS FIGHT!' : '⚔️ BOSS FIGHT!';
  const subtitle = isHinglish
    ? 'Ravana ko final hit! Dekh magic…'
    : 'Final blows on Ravana… watch the magic.';
  const winTitle = isHinglish ? '💥 RAVANA DOWN! JAI HO!' : '💥 RAVANA DEFEATED!';
  const winSub = isHinglish
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Boss fight celebration"
        onClick={onClose}
      >
        <div
        ...
        >
          ...
          <div className="px-6 sm:px-8 pt-8 pb-6">
            ...
            <div className="mt-6 relative rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
              {/* Subtle moving glow */}
              <div className="absolute inset-0 kq-boss-aurora" />

              {/* Health */}
              <div className="relative z-10 px-4 pt-4">
                <div className="flex items-center justify-between text-[11px] text-white/60">
                  <span>{isHinglish ? 'Boss HP' : 'Boss HP'}</span>
                  <span>{phase === 'reward' ? '0%' : `${Math.round(healthPct)}%`}</span>
                </div>
                <div className="mt-2 h-2 rounded-full bg-white/10 overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-red-400 via-rose-500 to-amber-400 transition-[width] duration-300"
              ...
              <div className="relative z-10 px-4 pb-6 pt-5 flex items-center justify-center min-h-[220px]">
                <div className="relative">
                  ...
                  <div
                    className={
                      `kq-ravana ${phase === 'hits' ? 'kq-ravana-hit' : ''} ${phase === 'explode' ? 'kq-ravana-explode' : ''}`
                    }
                  >
                    <RavanaSprite isHinglish={isHinglish} isDark={isDark} />
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Reward */}
            <div className={`mt-5 transition-all ${phase === 'reward' ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3 pointer-events-none'} kq-boss-reward`}>
                ...
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="rounded-xl bg-white/5 border border-white/10 px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[12px] text-white/60">{isHinglish ? 'Mudra Reward' : 'Coins Reward'}</span>
                    </div>
                  ...
                  <div className="rounded-xl bg-white/5 border border-white/10 px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">🎉</span>
                      <span className="text-[12px] text-white/60">{isHinglish ? 'Boss Loot' : 'Boss Loot'}</span>
                    </div>
                    <span className="text-[13px] font-black text-white">{isHinglish ? 'Legend Feel' : 'Legend Feel'}</span>
                  </div>
                </div>

                <button
                  onClick={onClose}
                  className={`mt-4 w-full py-3 rounded-2xl text-white font-black text-sm bg-gradient-to-r ${accent} hover:opacity-95 active:scale-[0.99] transition`}
                >
                  {isHinglish ? 'Wapas Ja (Continue) 💪' : 'Continue 💪'}
                </button>
              </div>
            </div>
          </div>
...
function addSlash(setSlashes: React.Dispatch<React.SetStateAction<Slash[]>>) {
  ...
  // Let CSS animation do the fade; also prune after 800ms
  window.setTimeout(() => {
...
function RavanaSprite({ isHinglish, isDark }: { isHinglish: boolean; isDark: boolean }) {
  // A compact, stylised SVG Ravana (10 heads). Purely decorative.
