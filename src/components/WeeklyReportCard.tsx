...
export function WeeklyReportCard({ report, autoOpen, onAutoOpenConsumed, onDismissFromDashboard }: Props) {
  ...
  }, [report.weekEnd, report.weekStart, report.questsCompleted, report.streakDays, report.xpEarned]);
  ...
  const grad = isHinglish
    ? 'bg-gradient-to-br from-rose-500 via-violet-500 to-indigo-500'
    : isDark
  ...
  const CardVisual = ({ compact }: { compact?: boolean }) => (
    ...
    >
      ...
      <div className="relative z-10 mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <Stat label="Punya" value={`${report.xpEarned}`} icon="✨" />
      ...
      <div className="relative z-10 mt-5 rounded-2xl bg-white/10 ring-1 ring-white/15 p-4">
        ...
        <p className="mt-1.5 text-[13px] leading-relaxed text-white/90">
          {isHinglish
            ? `Is hafte aapne ${report.questsCompleted} quests complete kiye, ${report.xpEarned} Punya kamaya, aur ${report.streakDays}-din ki Tapasya streak maintain ki.`
  ...
  return (
    <>
      ...
      <div className={`${cardShell} rounded-2xl p-4 backdrop-blur-xl shadow-sm`}>
        <div className="flex items-center justify-between gap-3 mb-3">
          <div>
            <p className={`text-[11px] font-semibold ${ts}`}>Weekly Progress</p>
            <p className={`text-sm font-bold ${tp}`}>Weekly Report Card</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setOpen(true)}
              className={`px-3 py-1.5 rounded-xl text-[12px] font-semibold transition-all ${
                isDark ? 'bg-white/[0.03] text-slate-200 hover:bg-white/[0.05]' : 'bg-slate-900/5 text-slate-700 hover:bg-slate-900/10'
              }`}
            >
              Open
            </button>
            <button
              ...
              className={`p-2 rounded-xl transition-all ${isDark ? 'hover:bg-white/[0.05]' : 'hover:bg-slate-900/5'}`}
              title="Dismiss"
            >
              <X size={16} />
            </button>
      ...
      {open ? (
        <div className="fixed inset-0 z-[100]">
          ...
          {/* TOP-ANCHORED container (not centered, not bottom) */}
          <div className="fixed inset-0 overflow-y-auto">
            <div className="min-h-full w-full flex items-start justify-center p-3 sm:p-6">
              {/* push the modal slightly from top so it never looks stuck */}
              <div className={`relative w-full max-w-[720px] mt-2 sm:mt-6 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                <div className={`rounded-3xl p-4 sm:p-5 ${cardShell} backdrop-blur-xl shadow-2xl`}>
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div>
                      <p className={`text-[11px] font-semibold ${ts}`}>Screenshot & share</p>
                      <p className={`text-sm font-bold ${tp}`}>Weekly Progress Card</p>
                    </div>
                    <button
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <p className={`text-[11px] ${ts}`}>
                      {isHinglish
                        ? 'Tip: Is card ka screenshot lo aur WhatsApp/Instagram pe share karo.'
                        : 'Tip: Take a screenshot of the card and share on WhatsApp/Instagram.'}
                    </p>
                ...
                <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-[110]">
                  <div
                    className={`px-4 py-2 rounded-full text-[12px] font-semibold shadow-lg ${
                      isDark ? 'bg-white/[0.06] text-slate-200 border border-white/[0.08]' : 'bg-white text-slate-800 border border-slate-200'
                    }`}
                  >
                    {toast}
                  </div>
